...
# As you have probably guessed, the t-test is based on the Student's T distribution.
# The Student's T is similar to a standard N(0,1) but with fatter tails,
# as determined by William Sealy Gossett in 1908, publishing under the pseudonym "Student". 
# Gossett was interested in finite sample statistics, and the t-test is accordingly
# intended for use when the number of samples is moderate (as is usually the case in real data).
# Under the null hypothesis of no difference (between a theorized and a measured mean, or between two sample means),
# the t statistic (difference between the means divided by the standard deviation) has a Student's T distribution,
# wit n-1 degrees of freedom (or 2n-2 for a two-sample test). 

# The Student's T distribution is generated by
?dt

# We can plot out, and compare with the normal, as so:
shared_mean<-0 # our data will have the indicated mean
shared_sd<-1 # and variance

# For the normal, start by creating a sequence of values based on population mean and standard deviation
pNormalT<-
  ggplot(data.frame(x = c(-4, 4)*shared_sd+shared_mean), aes(x = x)) +
  stat_function(fun = dnorm, args=list(mean=shared_mean, sd=shared_sd), colour="black")+
  stat_function(fun = dt, args=list(df=2), colour="red")+
  stat_function(fun = dt, args=list(df=5), colour="blue")+
  theme_classic()+
  theme(text=element_text(size=16), 
        plot.title=element_text(hjust=0.5, size=16),
        legend.title=element_text(size=20),
        legend.text=element_text(size=14))+
  labs(title="Normal and Student's T pdfs")
pNormalT

# We can also plot the cumulative density function (cdf):
pNormalTC<-
  ggplot(data.frame(x = c(-4, 4)*shared_sd+shared_mean), aes(x = x)) +
  stat_function(fun = pnorm, args=list(mean=shared_mean, sd=shared_sd), colour="black")+
  stat_function(fun = pt, args=list(df=2), colour="red")+
  stat_function(fun = pt, args=list(df=5), colour="blue")+
  theme_classic()+
  theme(text=element_text(size=16), 
        plot.title=element_text(hjust=0.5, size=16)) + 
  labs(title="Normal and Student's cdfs")
pNormalTC
plot_grid(pNormalT, pNormalTC, nrow = 2)
