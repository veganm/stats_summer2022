...
pacman::p_load(tidyverse, ggpubr, cowplot, grid, gridExtra, datasets, multcomp, car, mvnormtest)
...
(1a) There are *n* predicted points, but they are all from the same model. This model has 2 parameters and is associated with 2 df; however, one of these df is lost in this calculation because of the constraint that $\sum(\hat{y}_i-\bar{y}) = 0$. The df here is therefore 1, and the MEAN SQUARE REGRESSION is:
```{r}
msr<-ssr/1
msr
```
...
Note that there are *n* real data points, and we have lost one degrees of freedom in these data in calculating the mean, for a total *n-1* degrees of freedom. (We don't need to calculate the mean error here, as we won't be using it.)
...
To move forward, we need to know the expectations for MSE and MSR. Since (as we said above) MSE is an unbiased estimator of the variance of Y~i~ (recall that this is a constant), this means that $E[MSE]=E[\frac{SSE}{n-2}]=\sigma^2$. 
...
```
When we create the F statistic, under the null hypothesis, both the numerator and denominator have the same expected value; hence, the null hypothesis F statistic has an expectation of 1. If the statistic we observe is large (MSR>MSE, and therefore MSR/MSE > 1), the value for our statistic will be "surprising", and we reject the null hypothesis.
```{r}
# Code from last week's homework solution, demonstrating emergence of the F from the chi-squared distributions
df1<-1
df2<-18
...
myf_1_18 %>%
  ...
  theme(text=element_text(size=16), 
        plot.title=element_text(hjust=0.5, size=16),
        legend.title = element_blank()) + 
  labs(title="F(1,18) vs Chisq(df=1)/Chisq(df=18), n=1000")
```
